		...
		switch(attr){
			case 'subscribe':
				...
				if(ret.notice[attr].length >= 1){
					//var upinfo = ' 等更新了'+len+'个视频';
					//var trun_len = 18-upinfo.length;
					//var action = truncate(nick_name,trun_len,'...')+upinfo;
                    var action = '有'+ len +'条订阅更新';
					var label = span_subscription+'<a attr="subscribe" href="http://'+UC_DOMAIN+'/u/home?ut=3&mn='+len+'" target="_blank">'+action+'</a>';
                }
				break;
			case 'star_statuses':
				var nick_name = ret.notice[attr][0].n;
				...
				if(p == 'show_statuses'){
					...
					if(ret.notice[attr].length >= 1){
						...
						if(len == 1 || tmp.length ==1){
							...
							var html='<span class="updateinfo"><a target="_blank" attr="show_statuses" href="http://'+UC_DOMAIN+'/u/home?type=showfriends_timeline">'+action+'</a></span><span class="status" id="m_status"></span><span class="action">'+see+'<a class="flink" id="see" target="_blank" href="http://'+UC_DOMAIN+'/u/home?type=showfriends_timeline" attr="show_statuses">查看详情 </a></span>';
						}else{
							var html='<span class="updateinfo"><a attr="show_statuses" target="_blank" href="http://'+UC_DOMAIN+'/u/home?type=showfriends_timeline">'+action+'</a></span><span class="action"><a id="see" attr="show_statuses" target="_blank" href="http://'+UC_DOMAIN+'/u/home?type=showfriends_timeline">查看详情 </a></span>';
						}
