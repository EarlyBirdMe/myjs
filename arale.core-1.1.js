...
var arale = arale || {debug: araleConfig.debug || false, depSrc: araleConfig.depSrc || false, cache: {}, env: {combo_host: araleConfig.combo_host, combo_path: araleConfig.combo_path, css_combo_path: araleConfig.css_combo_path, locale: araleConfig.locale}, registerCoboPath: function (comboPath) {
...
arale.browser = function () {
    var engine = {ie: 0, gecko: 0, webkit: 0, khtml: 0, opera: 0, ver: null, name: null};
    ...
    return{name: browser.name, Engine: engine, Browser: browser, System: system, ver: function () {
...
arale.deps = (function () {
    ...
    Dependency.prototype = {moduleStatus: function (status) {
        if (status) {
            this.status = status
        }
        return this.status
    ...
    return{addDependency: function (moduleName, deps) {
    ...
    }, LOAD: LOAD, depsToModule: function (key) {
...
arale.module("arale.loader", function () {
    ...
    return{use: blockLoader, waituse: function () {
...
(function (arale) {
    var support = {};
    var script = document.createElement("script"), id = "script" + arale.now(), rscript = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, rnotwhite = /\S/;
...
arale.module("arale.string", (function () {
    ...
    arale.augment(CString, {trim: function () {
        ...
        if (this.str.slice(0, 4) == "rgb(") {
            ...
            if (this.str.slice(0, 1) == "#") {
                if (this.str.length == 4) {
                    for (var i = 1;
                         i < 4; i++) {
                        color += (this.str.charAt(i) + this.str.charAt(i)).toLowerCase()
                    }
                }
                if (this.str.length == 7) {
                    color = this.str.toLowerCase()
                }
            }
        }
...
arale.module("arale.hash", (function () {
    ...
    arale.augment(CHash, {each: function (fn, context) {
        ...
        if (!this.obj[key] || this.obj.hasOwnProperty(key)) {
            this.obj[key] = value
        }
        return this
...
arale.module("arale.array", function () {
    ...
    arale.augment(CArray, {each: function (callback, bind) {
        var target = this.arr;
        if (Array.prototype.forEach) {
        ...
        return this.inject([], function (array, item) {
            if (item instanceof Array) {
                return array.concat($A(item).flatten())
            }
            array.push(item);
            return array
        })
        ...
        if (this.arr.length === 4 && this.arr[3] === 0 && !array) {
            return"transparent"
        }
        var hex = [];
        for (var i = 0; i < 3; i++) {
...
arale.module("arale.dom", (function () {
    ...
    var tagWrap = {option: ["select"], tbody: ["table"], thead: ["table"], tfoot: ["table"], tr: ["table", "tbody"], td: ["table", "tbody", "tr"], th: ["table", "thead", "tr"], legend: ["fieldset"], caption: ["table"], colgroup: ["table"], col: ["table", "colgroup"], li: ["ul"]}, reTag = /<\s*([\w\:]+)/, masterNode = {}, masterNum = 0, masterName = "__araleToDomId";
    for (var param in tagWrap) {
        var tw = tagWrap[param];
        tw.pre = param == "option" ? '<select multiple="multiple">' : "<" + tw.join("><") + ">";
        tw.post = "</" + tw.reverse().join("></") + ">"
    }
    var specialAttr = $H({appendTo: function (node, value) {
    ...
    return{getViewportHeight: function (element) {
        ...
        element = element.node ? element.node : element;
        if (element == window || element == document || isBody(element)) {
            var scrollHeight = (document.compatMode != "CSS1Compat" || isSafari) ? document.body.scrollHeight : document.documentElement.scrollHeight, h = Math.max(scrollHeight, $D.getViewportHeight());
            return h
        }
        ...
        element = element.node ? element.node : element;
        if (element == window || element == document || isBody(element)) {
            var scrollWidth = (document.compatMode != "CSS1Compat" || isSafari) ? document.body.scrollWidth : document.documentElement.scrollWidth, w = Math.max(scrollWidth, $D.getViewportWidth());
            return w
        }
        ...
        element = element.node ? element.node : element;
        if (element == window || element == document || isBody(element)) {
            return{left: Math.max(document.documentElement.scrollLeft, document.body.scrollLeft), top: Math.max(document.documentElement.scrollTop, document.body.scrollTop)}
        }
        ...
        if (typeof(element.offsetParent) != "undefined") {
            ...
            for (var posX = 0, posY = 0;
                 element; element = element.offsetParent) {
        ...
        var offset = $D.getOffsets(element), scroll = $D.getScrolls(element);
        ...
        node = node.node || node;
        if (node.currentStyle) {
            return node.currentStyle[$S(property).camelCase()]
        }
        ...
        while ((element = element.parentNode) && !isBody(element)) {
            if (arale.dom.getComputedStyle(element, "position") != "static") {
                return element
            }
        }
        return null
...
arale.module("arale.node", (function () {
    var attributes = {html: "innerHTML", "class": "className", "for": "htmlFor", defaultValue: "defaultValue", text: (arale.browser.Engine.trident || (arale.browser.Engine.webkit && arale.browser.Engine.version < 420)) ? "innerText" : "textContent"};
    var inserters = {before: function (context, element) {
        if (context.nodeType == "NODE") {
    ...
    arale.augment(Node, {walk: function (walk, start, tag, all) {
        ...
        if (!this.hasClass(className)) {
            this.node.className = $S(this.node.className + " " + className).clean()
        }
        return this
        ...
        var props = {input: "checked", option: "selected", textarea: (arale.browser.Engine.webkit && arale.browser.Engine.version < 420) ? "innerHTML" : "value"};
        ...
        if (contents) {
            var ce = clone.getElementsByTagName("*"), te = this.node.getElementsByTagName("*");
        ...
        var get_style = function (style) {
            ...
            if (color) {
                value = value.replace(color[0], $S(color[0]).rgbToHex())
            }
            if (style == "opacity") {
            ...
            if (arale.isOpera() || (arale.isIE() && isNaN(parseFloat(value)))) {
                ...
                if (arale.isOpera() && String(value).indexOf("px") != -1) {
                    return value
                }
                if (/(border(.+)Width|margin|padding)/.test(style)) {
                    return"0px"
                }
            }
        ...
        for (var property in styles) {
            if (property == "opacity") {
                ...
                if (property == "class" || property == "className") {
                    ...
                    this.node.style[(property == "float" || property == "cssFloat") ? (arale.isUndefined(this.node.style.styleFloat) ? "cssFloat" : "styleFloat") : property] = styles[property]
                }
            }
        }
        return this
        ...
        return{width: $D.getViewportWidth(this.node), height: $D.getViewportHeight(this.node)}
    }, getDocumentSize: function () {
        return{width: $D.getDocumentWidth(this.node), height: $D.getDocumentHeight(this.node)}
    }, getScroll: function () {
        ...
        var obj = {left: position.left, top: position.top, width: $D.getViewportWidth(this.node), height: $D.getViewportHeight(this.node)};
        ...
        var obj = {left: new String(parseInt(pos.left) - (parseInt(this.getComputedStyle("margin-left")) || 0)) + "px", top: new String(parseInt(pos.top) - (parseInt(this.getComputedStyle("margin-top")) || 0)) + "px"};
...
$A(("blur focus focusin focusout load resize scroll unload click dblclick mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave change select submit keydown keypress keyup error").split(" ")).each(function (key) {
...
arale.module("arale.event.object", (function (arale) {
    ...
    arale.augment(EventObject, {_fix: function (event) {
        var that = this, originalEvent = this.originalEvent, l = props.length, prop, ct = this.currentTarget, ownerDoc = (ct.nodeType === 9) ? ct : (ct.ownerDocument || doc);
        while (l) {
            prop = props[--l];
            that[prop] = originalEvent[prop]
        }
        if (!that.target) {
            that.target = that.srcElement || doc
        }
    ...
    return{getEventObject: function (target, event) {
...
arale.module("arale.event.store", (function (arale) {
    ...
    arale.augment(store, {addHandler: function (id, type, fn) {
...
arale.module("arale.event.chain", (function (arale) {
    ...
    arale.augment(Action, {fire: function (e) {
    ...
    arale.augment(Chain, {addAction: function (fn) {
...
arale.module("arale.event.core", (function (arale) {
    var slice = Array.prototype.slice, array = arale.array, dom = arale.dom, store = arale.event.store.getStore(), doc = document;
    ...
    var getDispatcher = function (store, source, method) {
        ...
        if (isElement(source)) {
            ...
            return function () {
                var c = arguments.callee, t = c.target, params = [serId, method].concat(slice.call(arguments));
    ...
    return{connect: function (obj, event, context, method) {
    ...
    }, keys: keys, domReady: function (fn) {
        ...
        if (document.readyState && (arale.browser.Engine.gecko || arale.browser.Engine.webkit)) {
            ...
            if (document.readyState && arale.browser.Engine.trident) {
