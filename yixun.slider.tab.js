...
(function ($) {
    ...
    Tab.config = {effect: "base", direction: "x", autoPlay: false, playTo: 0, type: "mouseover", curClass: "current", link: false, stay: 2E3, delay: 200, lazy: false, merge: false, degradation: "base", animateTime: 300, easing: "swing", radius: 20, oninit: function () {
    }, onchange: function () {
    }};
        ...
        {base: {oninit: function () {
            ...
            $.each(self.target, function (i, elem) {
                ...
                $(tempImg).bind("load", function () {
                    ...
                    if (!!document.createElement("canvas").getContext) {
                        blurImg = $("<canvas>");
                        blurImg.css({position: "absolute", left: this.offsetLeft +
                            "px", top: this.offsetTop + "px", opacity: 0});
                        blurImg.attr({width: this.width, height: this.height});
                        var randomNum = Math.floor(Math.random() * 3);
                        $.blur({img: this, canvas: blurImg[0], process: function (r, g, b, x, y) {
                            var channel = [r, g, b];
                            if (x % 2 && y % 2) {
                        ...
                        blurImg.css({position: "absolute", left: this.offsetLeft + "px", top: this.offsetTop +
            ...
            if (c.direction == "x") {
                self.content.css({"position": "relative", "left": 0, "marginLeft": 0, "width": (c.totalWidth || self.step * self.target.length) + "px"});
                $.each(self.target, function (i, $obj) {
                    $obj.css({"float": "left"});
