...
showDialog = (function () {
    ...
    function getMaxH() {
        var a = getPageHeight(), wh = getWinHeight();
        return Math.max(a, wh)
    }

    function getMaxW() {
        var a = getPageWidth(), ww = getWinWidth();
    ...
    return{show: function (p) {
        ...
        if (!hasOverlay) {
            c = gEl(sOverLayId);
            if (c) {
                setStyle(c, {backgroundColor: this.bgcolor, display: 'block'})
            } else {
                this.overlay()
            }
        }
        ...
        var a = this.id, _h = getWinHeight(), _rt = null, _st = null, _iw, _ih, b;
        if (a != null && gEl(a)) {
            b = gEl(a), _iw = b.offsetWidth, _ih = b.offsetHeight;
            if (_ih >= _h) {
                longPop = true;
                setStyle(b, {position: 'absolute', top: '100px', marginLeft: '-' + _iw / 2 + 'px', marginBottom: '100px'})
            } else {
                longPop = false;
                isIE6 ? (b.style.position = 'absolute') : (b.style.position = 'fixed');
                setStyle(b, {marginTop: '-' + _ih / 2 + 'px', marginLeft: '-' + _iw / 2 + 'px'})
            }
            fixOverlay = function () {
                if (_rt) {
                    clearTimeout(_rt)
        ...
        var a = getMaxW(), _h = getMaxH(), _wh = getWinHeight(), _obj = gEl(this.id), _ih;
        ...
        if (_ih >= _wh) {
            longPop = true;
            setStyle(_obj, {position: 'absolute', top: '100px', marginTop: '0', marginBottom: '100px'})
        } else {
            longPop = false
        }
        if (isIE) {
        ...
        if (!longPop) {
            _obj = gEl(this.id);
            setStyle(_obj, {marginTop: parseInt(a - _obj.offsetHeight / 2) + 'px', top: '50%'})
        }
        ...
        setStyle(b, {marginTop: '-' + b.offsetHeight / 2 + 'px', marginLeft: '-' + b.offsetWidth / 2 + 'px'});
        this.fix_Overlay()
    }, hide: function (p) {
        var a = (typeof(p) === 'object') ? p : {}, b, c;
        ...
        setStyle(objOverLay, {backgroundColor: this.bgcolor, borderTop: '1px solid transparent', position: 'absolute', height: a + 'px', zIndex: '9998', width: '100%', left: '0', top: '0'});
        if (isIE6) {
            c = getMaxW();
...
/*  |xGv00|6cfce6c1321346edcd79327ae78a09ad */