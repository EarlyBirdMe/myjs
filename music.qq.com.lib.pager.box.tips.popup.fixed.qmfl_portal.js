...
if (MUSIC.userAgent.ie && MUSIC._setTimeout && MUSIC._setInterval) {
...
MUSIC.object = {map: function (object, scope) {
...
MUSIC.runTime = {isDebugMode: false, error: MUSIC.emptyFn, warn: MUSIC.emptyFn};
...
MUSIC.string = {RegExps: {trim: /^\s+|\s+$/g, ltrim: /^\s+/, rtrim: /\s+$/, nl2br: /\n/g, s2nb: /[\x20]{2}/g, URIencode: /[\x09\x0A\x0D\x20\x21-\x29\x2B\x2C\x2F\x3A-\x3F\x5B-\x5E\x60\x7B-\x7E]/g, escHTML: {re_amp: /&/g, re_lt: /</g, re_gt: />/g, re_apos: /\x27/g, re_quot: /\x22/g}, escString: {bsls: /\\/g, nl: /\n/g, rt: /\r/g, tab: /\t/g}, restXHTML: {re_amp: /&amp;/g, re_lt: /&lt;/g, re_gt: /&gt;/g, re_apos: /&(?:apos|#0?39);/g, re_quot: /&quot;/g}, write: /\{(\d{1,2})(?:\:([xodQqb]))?\}/g, isURL: /^(?:ht|f)tp(?:s)?\:\/\/(?:[\w\-\.]+)\.\w+/i, cut: /[\x00-\xFF]/, getRealLen: {r0: /[^\x00-\xFF]/g, r1: /[\x00-\xFF]/g}, format: /\{([\d\w\.]+)\}/g}, commonReplace: function (s, p, r) {
    ...
    return MUSIC.string.listReplace((str + ""), {'&amp;': t.re_amp, '&lt;': t.re_lt, '&gt;': t.re_gt, '&#039;': t.re_apos, '&quot;': t.re_quot});
}, escString: function (str) {
    var t = MUSIC.string.RegExps.escString, h = MUSIC.string.RegExps.escHTML;
    return MUSIC.string.listReplace((str + ""), {'\\\\': t.bsls, '\\n': t.nl, '': t.rt, '\\t': t.tab, '\\\'': h.re_apos, '\\"': h.re_quot});
    ...
    return MUSIC.string.listReplace((str + ""), {'<': t.re_lt, '>': t.re_gt, '\x27': t.re_apos, '\x22': t.re_quot, '&': t.re_amp});
...
MUSIC.object.extend(String.prototype, {trim: function () {
    ...
    var len = str.length, i = Math.min(Math.floor(bitLen / 2), len), cnt = MUSIC.string.getRealLen(str.slice(0, i));
    ...
    return this.replace(/&#38;?/g, "&amp;").replace(/&amp;/g, "&").replace(/&#(\d+);?/g,function (a, b) {
...
MUSIC.event = {KEYS: {BACKSPACE: 8, TAB: 9, RETURN: 13, ESC: 27, SPACE: 32, LEFT: 37, UP: 38, RIGHT: 39, DOWN: 40, DELETE: 46}, _eventListDictionary: {}, _fnSeqUID: 0, _objSeqUID: 0, addEvent: function (obj, eventType, fn, argArray) {
    var cfn, res = false, l;
    if (!obj) {
        return res;
    }
    ...
    var cfn = fn, res = false, l = MUSIC.event._eventListDictionary, r;
    if (!obj) {
        return res;
    }
    if (!fn) {
...
MUSIC.dom = {getById: function (id) {
    ...
    var box = MUSIC.dom.getPosition(elem) || {left: 0, top: 0};
    return[box.left, box.top];
}, getSize: function (elem) {
    var box = MUSIC.dom.getPosition(elem) || {width: -1, height: -1};
    ...
    MUSIC.dom.setStyle(elem, {left: (parseInt(x) || 0) - _ml + "px", top: (parseInt(y) || 0) - _mt + "px"});
    ...
    el = MUSIC.dom.get(el);
    var _r = MUSIC.dom._SET_SIZE_RE, m;
...
MUSIC.object.extend(MUSIC.console, {_inited: false, _html: '<h5 id="log_head" class="qzfl_log_head"><button id="log_close">x</button>console</h5><ul id="log_list"></ul><div class="qzfl_log_foot"><div class="log_console">&gt;<input id="log_console"/></div></div>', _opened: false, TYPE: {DEBUG: 0, ERROR: 1, WARNING: 2, INFO: 3, PROFILE: 4}, _typeInfo: [
    ["qzfl_log_debug", "√"],
    ["qzfl_log_error", "!"],
    ["qzfl_log_warning", "-"],
    ["qzfl_log_info", "i"],
...
MUSIC.FormSender = function (actionURL, method, data, charset) {
    ...
    this.uri = actionURL;
    var default_data = {loginUin: g_user.getUin() || 0, hostUin: 0, format: 'fs', inCharset: 'GB2312', outCharset: 'GB2312', notice: 0, platform: 'yqq', needNewCode: 0};
    if (typeof(data) == "object") {
        MUSIC.object.extend(default_data, data || {});
    } else if (typeof(data) == 'string') {
        var map = {}, arr = data.split('&');
...
MUSIC.JsLoader.prototype._load = function (src, doc, opts) {
    var _ie = MUSIC.userAgent.ie, _doc = document, o = this, tmp, k, idp = MUSIC.JsLoader._idleInstancesIDQueue, _rm = MUSIC.dom.removeElement, _ae = MUSIC.event.addEvent, docMode = _doc.documentMode, _new = false, _js;
...
MUSIC.JSONGetter = function (actionURL, cname, data, charset, junctionMode) {
    ...
    var default_data = {loginUin: g_user.getUin() || 0, hostUin: 0, format: 'jsonp', inCharset: 'GB2312', outCharset: 'GB2312', notice: 0, platform: 'yqq', jsonpCallback: 'MusicJsonCallback', needNewCode: 0};
...
MUSIC.JSONGetter.pluginsPool = {"srcStringHandler": [], "onErrorHandler": [], "onRequestComplete": []};
...
MUSIC.JSONGetter.prototype.send = function (callbackFnName) {
    ...
    if (MUSIC.userAgent.ie && (MUSIC.userAgent.ie < 10) && !(MUSIC.userAgent.beta && navigator.appVersion.indexOf("Trident\/4.0") > -1)) {
        var df = document.createDocumentFragment(), sender = (MUSIC.userAgent.ie > 8 ? document : df).createElement("script");
...
MUSIC.widget.trackServ = {_arrSongAttr: ["mid", "msong", "msingerid", "msinger", "malbumid", "malbum", "msize", "minterval", "mstream", "mdownload", "msingertype", "size320", "size128", "mrate", "gososo", "sizeape", "sizeflac", "sizeogg", "sizetac", "downstatus", "mmid", "msingermid", "malbummid"], _arrSongAttrSoso: ["mid", "msong", "malbum", "msinger", "msize", "mformat", "mrate", "minterval", "msongurl", "msingerid", "malbumid", "mdownload", "mshowurl"], getSongData: function (elm) {
    ...
    top.g_mymusicAlbumMod.addSongs2Dir({uin: uin, midlist: _music.mmid, typelist: "13", dirid: dirinfo.dirid}, function () {
    });
        ...
        g_popup.init({container: MUSIC.lang.toArray(_es), class_mod: "pop_fav_music", selectTpl: '<div class="global_tip_content %(class_mod)"><h5>收藏到歌单：</h5><div class="pop_fav_music_more">%(content)</div></div>', optionTpl: '<li><a href="javascript:;">%(text)</a></li>', arrow: "top", eventType: 'click', curIndex: -1, isToggle: 0, contents: function (callback) {
            ...
            top.g_mymusicAlbumMod.getList(uin, function (list) {
                ...
                for (var i = 0, len = list.length; i < len; i++) {
                    var dirinfo = list[i];
                    if (uin != dirinfo.owner.uin || (dirinfo.dirid >= 201)) {
                        continue;
                    }
    ...
    var s1 = albummid.substr(albummid.length - 2, 1), s2 = albummid.substr(albummid.length - 1, 1), url = "http://y.qq.com/y/static/album/" + s1 + "/" + s2 + "/" + albummid + ".js", j = new MUSIC.JSONGetter(url, "albuminfo", null, "utf-8", false);
    ...
    var url = "http://s.plcloud.music.qq.com/fcgi-bin/fcg_id_jump_mid.fcg?g_tk=" + getACSRFToken(), p = (typeof(opt.singerid) !== "undefined" && opt.singerid > 0) ? "singerid" : ((typeof(opt.albumid) !== "undefined" && opt.albumid > 0) ? "albumid" : ""), j;
    if (p == "") {
        return false;
    }
...
MUSIC.widget.user = {getUin: function () {
    ...
    var url = "http://portalcgi.music.qq.com/fcgi-bin/music_mini_portal/fcg_getuser_infoEx.fcg?outCharset=utf-8&rnd=" + new Date().valueOf(), j = new MUSIC.JSONGetter(url, "userinfo", null, "utf-8", false);
    ...
    var _sd = new MUSIC.JsLoader();
    _sd.onload = function () {
        var minipayObj = cashier.dialog.buy({codes: 'xxzxyy', debug: false, aid: aid, amount: defaultmonth, defaultChannel: cm, skin: 'minipay9', height: 400, onSuccess: function () {
            UserInfoCookie.clear();
        }});
    };
    ...
    var _sd = new MUSIC.JsLoader();
    _sd.onload = function () {
        var minipayObj = cashier.dialog.buy({codes: 'xxzxyy', debug: false, aid: aid, amountType: 'year', amount: 1, skin: 'minipay9', height: 400, onSuccess: function () {
            UserInfoCookie.clear();
        }});
    };
    _sd.load("http://imgcache.qq.com/bossweb/ipay/js/api/cashier.js", null, {charset: 'utf-8'});
...
MUSIC.widget.tips.popup = (function () {
    var $ = MUSIC, $C = $.css, $D = $.dom, $E = $.event, _this = $.widget.tips, _popup_tpl = ['<div class="global_tip_arrow" style="%(style_arrow)"></div>', '%(content)'].join(''), _popup_2_tpl = ['<div class="global_tip_arrow" style="%(style_arrow)"></div>', '<div class="global_tip_content">', '%(content)', '</div>'].join(''), _timerScroll = null, _timerTips = null;
    var _isLoadCss = true;
    ...
    function show2(options) {
        var _opt = {container: null, arrow: "", arrow_pos: 0, width: 0, contents: "", tipsClass: 'global_tip_1'};
        ...
        _insertCss(function () {
            ...
            showElement(_e);
            var _tPos = $D.getPosition(_container), _fPos = $D.getPosition(_e), _pos = {};
    ...
    return{show: show, show2: show2, hide: hide, init: init};
})();
...
function test(obj) {
    if (isfirst) {
        g_popup.init({container: obj, eventType: "hover", arrow: "top", width: "100px", class_mod: "global_tip_content rank_rule", curIndex: 1, showDelay: 500, hideDelay: 500, contents: [
            {text: "test1", click: 'alert'},
            {text: "test2", click: 'alert'},
            {text: "test3", click: 'alert'}
        ]});
        g_popup.init({container: "divsonglist1", eventType: "click", arrow: "top", width: "100px", class_mod: "filter_pop", contents: [
            {text: "test4", click: 'alert'},
            {text: "test5", click: 'alert'},
            {text: "test6", click: 'alert'}
        ]});
        isfirst = 0;
    }
}
...
MUSIC.widget.statistics = {initPvJs: function (callback) {
    ...
    var reg_map = {'y/static/index.html': 1, 'static/singer/': 2, 'static/album/': 3, 'static/toplist/': 4, 'static/mv/': 5, 'static/mymusic/': 6, 'soso.music.qq.com': 7, "mv_play.html": 8, "static/topic/": 9, "static/taoge/": 11, "static/qplay/": 10};
    for (var reg in reg_map) {
        var r = new RegExp(reg);
        if (r.test(window.location.href))
            return reg_map[reg];
    ...
    var _map = {0: 'TOP', 1: 'INDEX', 2: 'SINGER', 3: 'ALBUM', 4: 'TOPLIST', 5: 'MV', 6: 'MYMUSIC', 7: 'SEARCH', 8: 'MVDETAIL', 9: 'TOPIC', 10: 'QPLAY', 99: 'OTHER'};
...
(function (q) {
    var commurl = 'http://c.isdspeed.qq.com/code.cgi', urlParse = /^http:\/\/([\s\S]*?)(\/[\s\S]*?)(?:\?|$)/, pingSender = q.pingSender, collector = [], timer, isreportG = Math.random() * 1000 < 1, isreportP = Math.random() * 10 < 1, uin = typeof g_user.getUin == 'undefined' ? 0 : g_user.getUin(), duration = 1000, each = q.object.each;
    ...
    function valueStat(domain, cgi, type, code, time, rate, uin, exts) {
        ...
        if (typeof exts.unshift == 'function') {
            ...
            while (exts.length) {
                ...
                param.push([i + 1, 2].join('_') + '=' + c[1] + '?platform=' + platform);
    ...
    each(['JSONGetter', 'FormSender'], function (n) {
        ...
        if (q[n] && q[n].pluginsPool) {
            ...
            q[n].pluginsPool.onRequestComplete.push(function (th) {
                var u = toabs(th._uri || th.uri), mtch = u.match(urlParse), url = mtch[2], domain = mtch[1];
...
MUSIC.widget.Storage = {helperUrl: MUSIC.config.StorageHelperPage, ifrCallback: null, instance: null, getInstance: function () {
...
MUSIC.widget.Storage.create = function (cb, opt) {
    ...
    opt = opt || {};
    var db = null, dbname = opt.dbname || "music_database", defaultDomain = opt.domain || document.domain, helperUrl = opt.helper || MUSIC.widget.Storage.helperUrl, share = opt.share || false, _clientStore = ["globalStorage", "localStorage", "userData"];
...
MUSIC.widget.hashHistory = (function () {
    ...
    function getSimpleUrl(url) {
        ...
        } else if (/\/y\/static\/album\/([^#?$.]*)\/([^#?$.]*)\/([^#?$.]*)\.html/.test(url)) {
            hash = "type=album&mid=" + RegExp.$3;
        ...
        } else if (/\/y\/static\/album\/song_detail.html#songmid=([^#?$.]*)/.test(url) || /s\.plcloud\.music\.qq\.com\/fcgi-bin\/fcg_yqq_song_detail_info.fcg\?songmid=([^#?$.]*)/.test(url)) {
...
MUSIC.widget.mid = (function () {
    ...
    function getSingerPic(op) {
        op = op || {};
        var picNomal = "http://imgcache.qq.com/music/photo/singer/%(singerIdMod100)/%(picsize)_singerpic_%(singerId)_0.jpg", pic300 = "http://imgcache.qq.com/music/photo/singer_300/%(singerIdMod100)/300_singerpic_%(singerId)_0.jpg", picMid = "http://imgcache.qq.com/music/photo/mid_singer_%(picsize)/%(s1)/%(s2)/%(mid).jpg", picRtn = "http://imgcache.qq.com/mediastyle/y/img/cover_mine_130.jpg", mid = op.mid || "", s1, s2;
        if (typeof(mid) !== 'undefined' && mid !== "" && mid !== "0") {
            s1 = mid.substr(mid.length - 2, 1);
            s2 = mid.substr(mid.length - 1, 1);
            picRtn = picMid.jstpl_format({s1: s1, s2: s2, mid: mid, picsize: op.type || 300});
        } else if (_opt.mid_flag == 0 && typeof(op.id) !== 'undefined' && op.id != 0) {
            if (op.type == 150 || op.type == 90 || op.type == 68) {
                picRtn = picNomal.jstpl_format({singerId: op.id, singerIdMod100: op.id % 100, picsize: op.type});
            } else {
                picRtn = pic300.jstpl_format({singerId: op.id, singerIdMod100: op.id % 100});
            }
        }
        return picRtn;
    };
    function getAlbumPic(op) {
        op = op || {};
        var picNomal = "http://imgcache.qq.com/music/photo/album/%(albumIdMod100)/%(picsize)_albumpic_%(albumId)_0.jpg", pic300 = "http://imgcache.qq.com/music/photo/album_300/%(albumIdMod100)/300_albumpic_%(albumId)_0.jpg", picMid = "http://imgcache.qq.com/music/photo/mid_album_%(picsize)/%(s1)/%(s2)/%(mid).jpg", picRtn = "http://imgcache.qq.com/mediastyle/y/img/cover_mine_130.jpg", mid = op.mid || "", s1, s2;
        if (typeof(mid) !== 'undefined' && mid !== "" && mid !== "0") {
            s1 = mid.substr(mid.length - 2, 1);
            s2 = mid.substr(mid.length - 1, 1);
            picRtn = picMid.jstpl_format({s1: s1, s2: s2, mid: mid, picsize: op.type || 300});
            ...
            if (op.type == 150 || op.type == 90 || op.type == 68) {
                picRtn = picNomal.jstpl_format({albumId: op.id, albumIdMod100: op.id % 100, picsize: op.type});
            } else {
                picRtn = pic300.jstpl_format({albumId: op.id, albumIdMod100: op.id % 100});
            }
        }
        return picRtn;
    };
    function getSingerFile(op) {
        op = op || {};
        var urlId = "/%(singerIdMod100)/singer_%(singerId).html", urlMid = "/%(s1)/%(s2)/%(mid).html", urlRtn = urlId.jstpl_format({}), mid = op.mid || "", s1, s2;
        if (typeof(mid) !== 'undefined' && mid !== "" && mid !== "0") {
            s1 = mid.substr(mid.length - 2, 1);
            s2 = mid.substr(mid.length - 1, 1);
            urlRtn = urlMid.jstpl_format({s1: s1, s2: s2, mid: mid});
        } else if (_opt.mid_flag == 0 && typeof(op.id) !== 'undefined' && op.id != 0) {
            urlRtn = urlId.jstpl_format({singerId: op.id, singerIdMod100: op.id % 100});
        }
        return urlRtn;
    };
    function getAlbumFile(op) {
        op = op || {};
        var urlId = "/%(albumIdMod100)/album_%(albumId)_1.html", urlMid = "/%(s1)/%(s2)/%(mid).html", urlRtn = urlId.jstpl_format({}), mid = op.mid || "", s1, s2;
        if (typeof(mid) !== 'undefined' && mid !== "" && mid !== "0") {
            s1 = mid.substr(mid.length - 2, 1);
            s2 = mid.substr(mid.length - 1, 1);
            urlRtn = urlMid.jstpl_format({s1: s1, s2: s2, mid: mid});
        } else if (_opt.mid_flag == 0 && typeof(op.id) !== 'undefined' && op.id != 0) {
            urlRtn = urlId.jstpl_format({albumId: op.id, albumIdMod100: op.id % 100});
        }
        return urlRtn;
    };
    function getMidPath(op) {
        op = op || {};
        var pathId = "/%(albumIdMod100)/", pathMid = "/%(s1)/%(s2)/", urlRtn = pathId.jstpl_format({}), mid = op.mid || "", s1, s2;
        if (typeof(mid) !== 'undefined' && mid !== "" && mid !== "0") {
            s1 = mid.substr(mid.length - 2, 1);
            s2 = mid.substr(mid.length - 1, 1);
            urlRtn = pathMid.jstpl_format({s1: s1, s2: s2});
        } else if (_opt.mid_flag == 0 && typeof(op.id) !== 'undefined' && op.id != 0) {
            urlRtn = pathId.jstpl_format({albumIdMod100: op.id % 100});
        }
        return urlRtn;
    };
    function getID(op) {
        op = op || {};
        var idRtn = op.id || 0, mid = op.mid || "";
        if (typeof(mid) !== 'undefined' && mid !== "" && mid !== "0") {
            idRtn = mid;
        }
        return idRtn;
    };
    return{init: init, getSingerPic: getSingerPic, getSingerFile: getSingerFile, getAlbumPic: getAlbumPic, getAlbumFile: getAlbumFile, getMidPath: getMidPath, getID: getID, checkMid: checkMid};
})();
var g_mid = MUSIC.widget.mid;
/*  |xGv00|480d5705f20548536bb363ce700d0c84 */